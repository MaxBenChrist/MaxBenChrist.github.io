...
Inside Kafka, the message stream is divided into topics to make sure that messages are only passed to consumers that subscribed to it. The Messages itself are serialized, stateless objects, you can think of them as being lines of a log file, full .txt files or .csv tables. It is recommended to use small objects because Kafka is well optimized for shorter messages, for example the engineers of Linkedin agreed on a limit of 1 MB per message. So it is probably not a good idea to stream full images or videos into Kafka without splitting them into small chunks.
...
### What it is interesting about Apache Kafka?
...
But if you want to establish a SSL connection from Python, you have to first make sure you have the needed certificates and private keys in JKS containers as explained in the instructions from above.
Then, there are two ways to access the certificates from Python:

*    Export the certificates and keys from the JKS container to PEM format to use them inside ```kafka-python```
...
It is done, now you are able to connect establish SSL connections and write your own consumer and producers with either the ```kafka-python``` or ```pykafka``` packages.
